...
export default class DayPickerRangeController extends React.PureComponent {
  ...
  onDayClick(day, event) {
    ...
    if (focusedInput === START_DATE || !focusedInput) { nextStart = day; nextEnd = null; nextFocus = END_DATE; }
      ...
      else if (startDate && compareDates(day, addDays(startDate, minimumNights)) >= 0) { nextEnd = day; }
      else return;
  ...
  getFirstFocusableDay(month) { const { startDate, endDate, focusedInput, numberOfMonths } = this.props; let day = focusedInput === END_DATE ? (endDate || startDate || startOfMonth(month)) : (startDate || startOfMonth(month)); if (this.isBlocked(day)) { const end = endOfMonth(addMonths(month, numberOfMonths - 1)); while (day && end && compareDates(day, end) <= 0) { day = addDays(day, 1); if (day && !this.isBlocked(day)) break; } } return day; }
  ...
  deleteModifierFromRange(updated, start, end, modifier) { let result = updated; let cursor = start; while (cursor && end && compareDates(cursor, end) <= 0) { result = this.deleteModifier(result, cursor, modifier); cursor = addDays(cursor, 1); } return result; }
  ...
  doesNotMeetMinimumNights(day) { const { startDate, minimumNights } = this.props; return Boolean(startDate && !this.props.endDate && compareDates(day, addDays(startDate, minimumNights)) < 0); }
  doesNotMeetMinNightsForHoveredStartDate(day, hoverDate) { const nights = this.props.getMinNightsForHoverDate(hoverDate) || this.props.minimumNights; return Boolean(hoverDate && compareDates(day, addDays(hoverDate, nights)) < 0); }
  ...
  isFirstPossibleEndDateForHoveredStartDate(day, hoverDate) { return Boolean(hoverDate && compareDates(day, addDays(hoverDate, this.props.minimumNights)) === 0); }
