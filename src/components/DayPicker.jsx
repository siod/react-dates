...
class DayPicker extends React.PureComponent {
  ...
  constructor(props) {
    super(props);
    const currentMonth = startOfMonth(props.hidden ? todayISO() : props.initialVisibleMonth()) || todayISO();
    const focusedDate = props.getFirstFocusableDay ? props.getFirstFocusableDay(currentMonth) : startOfMonth(currentMonth);
  ...
  setCalendarMonthWeeks(month) { this.calendarMonthWeeks = []; let current = addMonths(month, -1); for (let i = 0; i < this.props.numberOfMonths + 2; i += 1) { this.calendarMonthWeeks.push(getCalendarMonthWeeks(current, { firstDayOfWeek: this.getFirstDayOfWeek(), enableOutsideDays: true }).length); current = addMonths(current, 1); } }
  getWeekHeaders() { return getWeekdayLabels(optionsFor(this.props, this.props.weekDayFormat)); }
  getFocusedDay(month) { const candidate = this.props.getFirstFocusableDay?.(month) || startOfMonth(month); return candidate && isCanonicalDate(candidate) ? candidate : startOfMonth(month); }
  ...
  onFinalKeyDown(event) {
    ...
    switch (event.key) {
      case 'ArrowUp': event.preventDefault(); next = addDays(next, -7); break;
      case 'ArrowDown': event.preventDefault(); next = addDays(next, 7); break;
      case 'ArrowLeft': event.preventDefault(); next = addDays(next, rtl ? 1 : -1); break;
      case 'ArrowRight': event.preventDefault(); next = addDays(next, rtl ? -1 : 1); break;
      ...
      case 'PageUp': event.preventDefault(); next = addMonths(next, -1); break;
      case 'PageDown': event.preventDefault(); next = addMonths(next, 1); break;
  ...
  onPrevMonthClick(event) { event?.preventDefault(); const month = addMonths(this.state.currentMonth, -1); this.setState({ currentMonth: month, focusedDate: null }, () => this.props.onPrevMonthClick(month)); }
  onNextMonthClick(event) { event?.preventDefault(); const month = addMonths(this.state.currentMonth, 1); this.setState({ currentMonth: month, focusedDate: null }, () => this.props.onNextMonthClick(month)); }
  onMonthChange(month) { this.setState({ currentMonth: startOfMonth(month) || month, focusedDate: null }, () => this.props.onMonthChange(month)); }
  onYearChange(month) { this.setState({ currentMonth: startOfMonth(month) || month, focusedDate: null }, () => this.props.onYearChange(month)); }
  ...
  getPrevScrollableMonths(event) { event?.preventDefault(); this.props.onGetPrevScrollableMonths(event); this.setState(({ currentMonth, scrollableMonthMultiple }) => ({ currentMonth: addMonths(currentMonth, -this.props.numberOfMonths), scrollableMonthMultiple: scrollableMonthMultiple + 1 })); }
  renderNavigation(direction) { const p = this.props; return <DayPickerNavigation {...p} navPosition={p.navPosition} navPrev={p.navPrev} navNext={p.navNext} onPrevMonthClick={this.onPrevMonthClick} onNextMonthClick={this.onNextMonthClick} showNavPrevButton={!p.noNavPrevButton && direction !== 'next_nav'} showNavNextButton={!p.noNavNextButton && direction !== 'prev_nav'} />; }
  render() {
